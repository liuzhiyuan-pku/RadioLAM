...
class Router(nn.Module):
    def __init__(self, gen_num):
        ...
        self.net.fc = nn.Linear(512, gen_num)
...
class MoG_FT(nn.Module):
    def __init__(self, gen_num, n_steps, shared_cfg, router_cfg, router_path, weight_path):
        super().__init__()
        self.gen_num = gen_num
        self.RGen_path_list = None

        self.Router = Router(gen_num)
        self.Router.load_state_dict(torch.load(router_path))

        self.GEN_list = []
        self.Tag_list = []
        for tag, path in weight_path[1:]:
            self.Tag_list.append(tag)
            RGen = UNet(n_steps, router_cfg['img_shape'], router_cfg['channels'], router_cfg['pe_dim'],
               router_cfg.get('with_attn', False), router_cfg.get('norm_type', 'ln'))
            RGen.load_state_dict(torch.load(path))
            self.GEN_list.append(RGen)

        SGen = UNet(n_steps, shared_cfg['img_shape'], shared_cfg['channels'], shared_cfg['pe_dim'],
                    shared_cfg.get('with_attn', False), shared_cfg.get('norm_type', 'ln'))
        SGen.load_state_dict(torch.load(weight_path[0][1]))

        self.GEN_list.append(SGen)
        self.Tag_list.append(weight_path[0][0])

    def save_model(self,accelerator,ckpt_path, ch, best_epoch, target_height, sparse_num, best_loss):
        for tag, net in zip(self.Tag_list, self.GEN_list):
            unwrapped_model = accelerator.unwrap_model(net)
            torch.save(unwrapped_model.state_dict(), os.path.join(ckpt_path,
                                                                  f"FT_{tag}_{ch}E{best_epoch}H{target_height}S{sparse_num}L{best_loss:.5f}.pth"))

        unwrapped_model = accelerator.unwrap_model(self.Router)
        torch.save(unwrapped_model.state_dict(), os.path.join(ckpt_path,
                                                                  f"FT_Router_{ch}E{best_epoch}H{target_height}S{sparse_num}L{best_loss:.5f}.pth"))
    ...
    def forward(self, x, t):
        ...
        w0 = w[:,-1].unsqueeze(1)
        wi = w[:,:-1]
        w_cfg = (wi / (w0 + wi)).view(-1, self.gen_num-1, 1, 1)

        eps0 = self.GEN_list[-1](x, t)

        eps_list = []
        for gen in self.GEN_list[:-1]:
            eps_list.append(gen(x, t) - eps0)
        eps = eps0 + torch.mean(torch.cat(eps_list,dim=1)*w_cfg, dim=1, keepdim=True)

        return eps



class MoG_infer(nn.Module):
    def __init__(self, gen_num, n_steps, shared_cfg, router_cfg):
        super().__init__()
        self.gen_num = gen_num
        self.R_index = -1
        self.RGen_path_list = None
        ...
        self.SGen = UNet(n_steps, shared_cfg['img_shape'], shared_cfg['channels'], shared_cfg['pe_dim'],
               shared_cfg.get('with_attn', False), shared_cfg.get('norm_type', 'ln'))

        self.RGen = UNet(n_steps, router_cfg['img_shape'], router_cfg['channels'], router_cfg['pe_dim'],
               router_cfg.get('with_attn', False), router_cfg.get('norm_type', 'ln'))


    def load(self, Router_path, SGen_path, RGen_path_list):
        self.Router.load_state_dict(torch.load(Router_path))
        self.SGen.load_state_dict(torch.load(SGen_path))
        self.RGen_path_list = RGen_path_list

    def forward(self, x, t):

        w = self.Router(x[:1, 2:]).view(-1)
        i = torch.argmax(w[:-1])
        wi = w[i]
        w_cfg = wi / (w[-1] + wi) + (torch.randn((64,1,1,1)) * self.sigma).to(w.device)

        if self.R_index != i:
            self.RGen.load_state_dict(torch.load(self.RGen_path_list[i]))
            self.R_index = i

        eps = self.SGen(x, t)
        eps0 = self.RGen(x, t)

        eps = eps0 + w_cfg * (eps - eps0)

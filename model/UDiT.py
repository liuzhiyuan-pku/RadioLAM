...
class Context_simple_mlp(nn.Module):
    def __init__(self, in_features=2*64, out_features=None, act_layer=nn.GELU):
        super().__init__()
        self.ft = nn.Flatten()
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256, train_with_amp_fp16 = False):
        super().__init__()
        ...
        self.train_with_amp_fp16 = train_with_amp_fp16
...
class U_DiT(nn.Module):
    ...
    def __init__(
        ...
        self.t_embedder_1 = TimestepEmbedder(hidden_size, train_with_amp_fp16 = train_with_amp_fp16)
        # self.y_embedder_1 = LabelEmbedder(num_classes, hidden_size, class_dropout_prob)
        # self.y_embedder_1 = Context_simple_mlp(out_features=hidden_size)

        self.t_embedder_2 = TimestepEmbedder(hidden_size * 2, train_with_amp_fp16 = train_with_amp_fp16)
        # self.y_embedder_2 = LabelEmbedder(num_classes, hidden_size * 2, class_dropout_prob)
        # self.y_embedder_2 = Context_simple_mlp(out_features=hidden_size*2)

        self.t_embedder_3 = TimestepEmbedder(hidden_size * 4, train_with_amp_fp16 = train_with_amp_fp16)
        # self.y_embedder_3 = LabelEmbedder(num_classes, hidden_size * 4, class_dropout_prob)
        # self.y_embedder_3 = Context_simple_mlp(out_features=hidden_size*4)
    ...
    def initialize_weights(self):
        ...
        # nn.init.normal_(self.y_embedder_1.fc.weight, std=0.02)
        # nn.init.normal_(self.y_embedder_2.fc.weight, std=0.02)
        # nn.init.normal_(self.y_embedder_3.fc.weight, std=0.02)
    ...
    def forward(self, x, t, y):
        ...
        # y1 = self.y_embedder_1(y)  # (N, C, 1, 1)
        c1 = t1 # + y1  # (N, D, 1, 1)

        t2 = self.t_embedder_2(t)  # (N, C, 1, 1)
        # y2 = self.y_embedder_2(y)  # (N, C, 1, 1)
        c2 = t2 # + y2  # (N, D, 1, 1)

        t3 = self.t_embedder_3(t)  # (N, C, 1, 1)
        # y3 = self.y_embedder_3(y)  # (N, C, 1, 1)
        c3 = t3 # + y3  # (N, D, 1, 1)
